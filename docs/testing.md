...
- `src/renderer/src/stores/sessions-store.test.ts` verifies renderer snapshot ordering/availability gating, dispositions, queue restoration, editor revisions, unified host/epoch response binding, expired continuation suppression, same-file/same-cursor delayed-history fencing, and generation-safe UI projection.
