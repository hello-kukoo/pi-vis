...
- `resources/pi-session-host/state-authority.test.mjs` is the direct fault-injection gate. It tests direct snapshots, identity/epoch transitions, FIFO custody behind compaction/navigation, callback-time compaction getter reconciliation, submission dispositions, ESC target selection, queue-restoration attachment custody, editor revision rejection, and transition batches.
...
The normal Electron suite also runs the real SDK host with the exact test-only dev dependency `@earendil-works/pi-coding-agent@0.80.6`. `tests/e2e/support/real-sdk-host.mts` resolves that repository-local executable, verifies its reported version, and creates isolated home, settings, agent, workspace, extension, and one-level session-bucket directories. It never searches `PATH` or falls back to a developer installation. This does not alter production runtime selection: the application still launches the Pi binary chosen by the user.

The real-Pi suites mock only the model provider with `tests/e2e/support/scripted-openai-provider.mts`, an ordered OpenAI-compatible server bound to `127.0.0.1`. They exercise the real `AgentSession`, extensions, tools, persistence, host IPC, authority frames, renderer store, and Electron DOM. Mandatory journeys cover Composer keyboard and attachment custody, transformed and duplicate prompts, multi-chunk streaming, extension notifications/status/widgets/dialogs/editor mutation/errors/custom entries, settled tool cards and history parity, partial disconnect and terminal provider errors, automatic HTTP retry, real tool/retry/stream interruption, queued-message review, empty and successful manual compaction, post-compaction admission, and full process reload. Assertions include provider request history and persisted JSONL, so dispatch or Composer clearing alone never counts as success. `PIVIS_REAL_USER_HOST_SMOKE=1` remains an optional developer-resource smoke test; provider-spending and real-panel suites remain opt-in where documented.

A pinned-Pi upgrade should normally be: update the exact dependency and `PINNED_PI_VERSION`, update the compatibility note, run `npm ci`, then run `npm run test:full` (or at minimum `npm run test:e2e:real-pi` plus the unit gates). CI uses Node 22.19.0 because Pi 0.80.6 declares Node `>=22.19.0`, and runs render and Electron E2E under `xvfb-run` without credentials.
