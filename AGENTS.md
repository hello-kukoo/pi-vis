...
│       ├── xterm.ts         # buildXtermTheme(theme): xterm 16-color palette from a Theme's semantic colors (shared by the 3 terminal panels)
...
└── shared/                  # Shared types (imported by main, preload, and renderer)
    ...
    │   ├── tokens.ts        # COLOR_TOKENS vocabulary (26 semantic roles) + Zod ThemeSchema (id/name/appearance/colors/syntax ref|inline, optional piTheme overrides)
    │   ├── pi-theme.ts      # Bridge to pi's OWN theme vocabulary (~50 roles): PI_ROLES/PI_BG_ROLES, palette-agnostic PI_THEME_DEFAULTS (pi-role→token), buildPiThemeColors(theme) → {fg,bg} hex maps the SDK host installs as pi's active Theme
    │   ├── bundled.ts       # Imports the JSON below, parses each through ThemeSchema, exports BUNDLED_THEMES + DEFAULT_THEME_ID
    │   ├── themes/          # One pure-data JSON colorscheme per file (full Theme shape: id/name/appearance/colors/syntax, optional piTheme)
    │   │   ├── catppuccin-mocha.json / catppuccin-macchiato.json / catppuccin-frappe.json / catppuccin-latte.json
    │   │   └── gruvbox-material-dark.json
    │   └── index.ts         # buildThemeRegistry/resolveTheme (fallback to default) + piThemeForTheme (appearance → pi light/dark) + re-exports buildPiThemeColors/PI_ROLES/PI_THEME_DEFAULTS
...
- **Color-scheme sync (terminal/ANSI surfaces)**: every host-rendered surface — extension `theme.fg("text"|"muted"|"borderMuted"|…)`, the unified TUI, and `custom()` panels — resolves colors from pi's active `Theme` singleton. pi ships only **two** built-in themes (`dark`/`light`), which left widget/TUI text reading in pi's generic palette on any non-default scheme (Macchiato/Frappé/Gruvbox/custom). The SDK host closes that gap by building a pi `Theme` from pi-vis's **exact** palette and installing it as pi's active singleton, in two layers: (1) **`PIVIS_PI_THEME`** (`"dark"|"light"`) loads pi's built-in theme as a base via `initHostTheme` (the fallback if layer 2 fails, and the only signal on the RPC path); (2) **`PIVIS_PI_THEME_COLORS`** carries the active scheme's palette expressed in **pi's own ~50-role color vocabulary** (`piThemeColorsForSchemeId(id)` in `main/theme-loader.ts` → `buildPiThemeColors(theme)` in `shared/theme/pi-theme.ts`), so the host does `new pi.Theme(fg, bg, "truecolor")` and writes it to the `globalThis[THEME_KEY]` symbol every extension reads (`applyPiVisTheme` in `bootstrap.mjs`). `buildPiThemeColors` resolves each pi role from the theme's optional `piTheme` block (a hex literal OR a token NAME resolved against that theme's own palette — per-theme authoring) and otherwise falls back to a **palette-agnostic** default mapping `PI_THEME_DEFAULTS` (pi-role→token), so a theme with no `piTheme` block still renders coherently on pi's surfaces for ANY palette — Gruvbox gets Gruvbox-flavored syntax via its own `accent`/`success`/`warning`, nothing Catppuccin-anchored. Both env vars are set fresh per spawn in `getHostEnv()` (`ipc.ts`). The renderer's own xterm panels theme their background + 16-color palette from the resolved Theme via `buildXtermTheme(getTheme(id))` (`theme/xterm.ts`, used by CustomPanelHost/UnifiedTuiHost/LoginTerminal); this closes the loop for the truecolor content pi-tui emits. (The `pi --mode rpc` fallback path has no host, so it can't install a custom theme and loses fidelity there — accepted, since RPC is the legacy fallback.) **Live changes:** a scheme change reloads running sessions — `settings.set` detects a `colorScheme` change and calls `SessionRegistry.reloadRunningSessions()` (best-effort; busy/mid-turn sessions are skipped and re-theme on their next spawn), respawning each host with the new `PIVIS_PI_THEME` + `PIVIS_PI_THEME_COLORS` so extensions re-emit widgets in the new colors.
...
so it keeps its full-screen centered treatment. Overlays (diff/toast) live inside
